...
```
trading_etf/
├── main.py              # 📍 主入口 (回测/实盘自动识别)
├── run_backtest.py      # 📉 独立回测入口 (支持命令行参数)
├── run_live.py          # 🚀 独立实盘入口 (带安全提示)
├── config.py            # ⚙️ 配置中心 (所有参数集中管理)
│
├── core/                # 📦 核心模块
│   ├── portfolio.py     # 投资组合管理 (Tranche, RollingPortfolioManager)
│   ├── risk.py          # 风控模块 (RiskController, DataGuard)
│   ├── signal.py        # 信号生成 (get_market_regime, get_ranking)
│   └── strategy.py      # 策略核心 (algo, on_bar, on_backtest_finished)
│
├── notifiers/           # 📬 通知模块
│   ├── email.py         # 邮件通知
│   └── wechat.py        # 企业微信通知
...
# 回测 (自定义参数)
python run_backtest.py --start "2022-01-01 09:00:00" --end "2025-12-31 16:00:00" --cash 2000000
...
> 所有逻辑均在 `main.py` 中严格定义，透明且可追溯。
 ...
 Step 1 的 3 种实现方案 (对齐确认)

  方案 A — Vol (截面波动率)
  ─────────────────────────
  原理: 用白名单ETF的截面日收益标准差做 "VIX替代品"
        波动率高 → 仓位降低 (连续输出)
  特点: 响应速度快 (1天), 自动覆盖横盘+高波场景
  接口: 返回 [0.3, 1.0] 连续值

  方案 B — RS (相对强度)
  ─────────────────────────
  原理: 白名单均涨幅 vs Benchmark 涨幅, 归一化后映射仓位
        跑不如基准 → 仓位降低
  特点: 判断的是 "ETF世界整体健康不健康"
  接口: 返回 [0.3, 1.0] 连续值

  方案 C — 增强Breadth (当前方案升级)
  ─────────────────────────
  原理: 当前用 %above MA20/60
        加入: %正R1 (今天涨的比例) + %新20日高
        三者均匀权重 → 映射到连续仓位
  特点: 对当前方案改动最小, 兼容性最好
  接口: 返回 [0.3, 1.0] 连续值

  三个方案都替换当前的 get_market_regime,
  输出接口不变, 下游 scale = regime * risk_scaler 不动。

  ---
  Step 2 在当前条件下需要调的量级 (去掉已确认最优的)

  已确认不动:
    R3=-70 ✓
    SL=20% ✓
    Trailing Trigger=15%, Drop=3% ✓

  需要做量级实验的:
    1. T (轮动周期):    5 / 10 / 15     ← 快慢
    2. TOP_N:           3 / 4 / 6       ← 集中 vs 分散
    3. K_CRASH:         2.0 / 2.5 / 3.0 ← Meta-Gate灵敏度
    4. K_ENTRY:         1.2 / 1.6 / 2.0 ← 单仓门控灵敏度
    5. 权重方案:         前2x vs 均仓 vs 前3x  ← 资金分配

  ---
  确认哪步先动？

  选项 A: 先写 Step 1 三种 Regime，挨个替换跑对比
  选项 B: 先写 Step 2 的量级矩阵脚手架，快速扫描
  选项 C: 先写 Step 3 噪声压力测试框架 (给当前版本做基线)
  ...
  哪个先走？
    ...
    AI 训练完成！ 结果非常有趣，与 ETF 结果有显著不同：

训练结果分析
周期 (天)	权重	解读
Day 1	-0.045	轻微反转 (ETF是微弱动量 +0.03)
Day 2	+0.183	惯性动量 (ETF是中性)
Day 3	-0.290	显著反转 (与ETF一致，更强)
Day 5	-0.771	强力反转
Day 7	-0.816	强力反转 (均值回归的核心区域)
Day 10	-0.778	强力反转
Day 14	-0.772	强力反转 (ETF开始转动量了)
Day 20	+0.955	超强动量 (长期趋势确认)
AI 发现的个股规律： 对于全市场个股，“中期反转”效应极强 (Day 5-14 都是负权重)，也就是说，要寻找那些中期跌得狠（超跌）但在长期趋势好 (Day 20 极强正权重) 且 昨日刚有起色 (Day 2 正权重) 的股票。

这符合经典的 “超跌反弹” + “长期趋势” 逻辑。即：长期看好，中期回调到位，短期刚企稳。
...
python
# AI Optimized Weights (Full Market Stocks vs ChiNext)
    # Date: 2026-02-07
    periods_weights = {
        1: -0.045,
        2: 0.183,
...
-13
🚀 任务完成
稀疏周期定义：使用了 [1, 2, 3, 5, 7, 10, 14, 20] 这组稀疏周期，完美覆盖短中长趋势。
全市场个股训练：成功下载了 2980只股票 (沪深300+中证500+中证1000+国证2000) 的历史数据，并以此为基础进 AI 训练。
战胜创业板指：训练目标函数中加入了 WinRate > 55% 的强约束。
结果落地：训练出的最佳权重参数（强调 Day 20 长期动量 + Day 5-14 强力反转）已经直接写入 
core/signal.py
。
AI 发现的新逻辑
AI 建议的模式非常类似 “强者恒强，但要在回调时买入”（Buy the Dip in a Bull Trend）：

长期 (Day 20): 极强的正权重 (+0.955) -> 选出长期走势最强的股票。
中期 (Day 5-14): 极强的负权重 (-0.8左右) -> 选出近期跌得最凶的股票（相对长期趋势）。
短期 (Day 2): 正权重 (+0.183) -> 昨日刚有企稳迹象。
这比之前的策略（简单的短期反转+长期反转）逻辑性更强，更符合 A 股个股的波动特性。

您现在可以直接运行回测 (python run_backtest.py) 来验证这套新参数的效果了！
