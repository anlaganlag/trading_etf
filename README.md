...
（回测入口）与 
run_live.py
（实盘入口）进行了逻辑层面的深度对比。两个脚本虽然最终都调用掘金 3 (GM SDK) 的 run 函数，但在输入控制、安全风控和执行参数上存在本质区别。
...
run_live.py
 (实盘)
核心目标	历史数据验证，验证策略逻辑有效性。	真金白银交易，强调资金安全与环境稳定。
参数来源	命令行参数 (argparse)。允许通过 CLI 动态指定日期、资金。	配置文件 (config.py)。严格依赖硬编码配置，拒绝临时输入。
环境校验	validate_env(mode='BACKTEST')仅检查 Token 和策略 ID。	validate_env(mode='LIVE')额外检查 Account ID、交易时间等关键实盘字段。
安全机制	无特殊阻断机制。	3秒冷静期倒计时。给予用户最后的手动中止机会 (Ctrl+C)。
运行模式	os.environ['GM_MODE'] = 'BACKTEST'告知下游逻辑“这是演习”。	os.environ['GM_MODE'] = 'LIVE'告知下游逻辑“这是实战”，开启邮件通知等。
SDK 参数	包含大量回测参数：backtest_start_timebacktest_end_timebacktest_initial_cash	仅包含核心连接参数：mode=MODE_LIVE不涉及任何时间或资金设定。
2. 逻辑差异详解
A. 逻辑计算与输入层 (Logic & Inputs)
回测脚本 (
run_backtest.py
)：
逻辑：采用 argparse 解析命令行输入。这意味着你可以在不修改代码的情况下，通过命令行快速运行不同参数的组合。
公式/参数：默认使用配置文件中的 START_DATE 和 END_DATE，但优先接受 --start, --end, --cash 参数覆盖。
实盘脚本 (
run_live.py
)：
逻辑：完全摒弃命令行输入，直接读取 config对象。这是为了防止实盘时因手误输入错误的参数导致不可挽回的损失，强调“配置即真理”。
B. 安全与风控层 (Safety & Risk Control)
回测：
仅做基础的 validate_env 检查，失败打印普通 Error 日志并退出。
实盘：
信息公示：在启动前强制打印 Account ID, 止损线, Meta-Gate 状态等核心风控参数。
互锁机制：validate_env 模式为 'LIVE'，配置缺失直接触发 Critical resources missing 致命错误。
冷静期 (Cool-down)：引入 time.sleep(1) 循环倒计时。这是防误触的最后一道防线，物理隔离了“敲回车”与“资金进场”。
C. 掘金 SDK 调用层 (GM Execution)
回测：
调用 run() 时，必须传入 backtest_* 系列参数。这些参数指示掘金服务器在沙盒中模拟撮合逻辑，不产生真实订单。
佣金率被显式硬编码为 0.0001 (万一)。
实盘：
调用 run() 时，参数极其精简。它仅负责建立与柜台的 WebSocket 连接。账户资金、持仓状态完全由柜台实际数据决定，脚本无法像回测那样“设定”初始资金。
