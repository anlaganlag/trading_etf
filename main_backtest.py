...
#牛熊全周期
# 由于掘金免费版/非专业版对基金行情数据有180天限制，调整日期范围
START_DATE='2025-11-01 09:00:00'
END_DATE='2026-02-03 16:00:00'
...
def init(context):
    ...
    if context.mode == MODE_LIVE:
        print("☁️ Live Mode: Fetching history from GM API (Last 170 days)...")
        
        # --- A. 获取标的行情 (Batch) ---
        all_symbols = list(context.whitelist)
        end_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        start_time = (datetime.now() - timedelta(days=170)).strftime('%Y-%m-%d %H:%M:%S')
        ...
        if not hasattr(context, 'prices_df') or context.prices_df.empty:
            ...
            # Calculate time range (Limited to 180 days total from today)
            start_dt = pd.Timestamp(START_DATE) - timedelta(days=50)
...
def get_ranking(context, current_dt):
    history = context.prices_df[context.prices_df.index <= current_dt]
    if len(history) < 30: return None, None
