...
# === 实验配置：宏观基准切换 ===
# HS300: 'SHSE.510300' | ChiNext(创业板): 'SZSE.159915' (ETF) or 'SZSE.399006' (Index)
MACRO_INDEX = 'SZSE.510300'
...
def init(context):
    ...
    # 2. Build Price Matrix & Load Macro Index
    if context.mode == MODE_LIVE:
        ...
        # --- B. 获取 Macro Index 行情 (Macro) ---
        try:
            macro_hd = history(symbol=MACRO_INDEX, frequency='1d', start_time=start_time, end_time=end_time, fields='close,eob', fill_missing='last', adjust=ADJUST_PREV, df=True)
            if not macro_hd.empty:
                macro_hd['eob'] = pd.to_datetime(macro_hd['eob']).dt.tz_localize(None)
                context.macro_hd = macro_hd.set_index('eob')['close'].sort_index()
                
                # 插入当前价格
                macro_current = current(symbols=MACRO_INDEX)
                if macro_current:
                    context.macro_hd.loc[today_dt] = macro_current[0]['price']
                print(f"Macro Index ({MACRO_INDEX}) Loaded: {len(context.macro_hd)} days (API).")
            else:
                context.macro_hd = None
                print(f"Warning: Macro Index ({MACRO_INDEX}) API data empty.")
        except Exception as e:
            context.macro_hd = None
            print(f"Warning: Failed to fetch Macro Index API: {e}")
        ...
        try:
            ...
            if not macro_hd.empty:
                macro_hd['eob'] = pd.to_datetime(macro_hd['eob']).dt.tz_localize(None)
                context.macro_hd = macro_hd.set_index('eob')['close'].sort_index()
                print(f"✓ Macro Index ({MACRO_INDEX}) Review: {len(context.macro_hd)} days (API).")
            else:
                context.macro_hd = None
                print(f"Warning: Macro Index ({MACRO_INDEX}) API data empty.")
        except Exception as e:
            context.macro_hd = None
            print(f"Warning: Failed to fetch Macro Index from API: {e}")
            
        # === CRITICAL SAFETY CHECK ===
        # If HS300 is missing, the Macro-Gate will fail (defaulting to 1.0 exposure),
        # causing a massive 40%+ drawdown. We must STOP if this happens.
        if context.macro_hd is None or context.macro_hd.empty:
            raise ValueError(f"❌ CRITICAL: Macro Benchmark ({MACRO_INDEX}) Data is MISSING! Strategies cannot run safely without it. Check API/Internet.")
        else:
             print(f"✅ Macro Shield Active: {MACRO_INDEX} Data Loaded ({len(context.macro_hd)} days)")
...
def get_market_regime(context, current_dt):
    ...
    if context.macro_hd is not None:
        # Pre-process: Ensure tz-naive
        if context.macro_hd.index.tz is not None:
             context.macro_hd.index = context.macro_hd.index.tz_localize(None)
             
        macro_hist = context.macro_hd[context.macro_hd.index <= current_dt]
        
        if len(macro_hist) > 120:
            current_price = macro_hist.iloc[-1]
            ma120 = macro_hist.tail(120).mean()
            
            # --- DEBUG BLOCK FOR CRASHES ---
            is_monday = current_dt.weekday() == 0
            
            if current_price < ma120:
                macro_multiplier = 0.5 # Bear
                if is_monday: debug_msg = f"[MACRO: BEAR] {MACRO_INDEX} Price {current_price:.2f} < MA120 {ma120:.2f} -> Scale 0.5"
            else:
                if is_monday: debug_msg = f"[MACRO: BULL] {MACRO_INDEX} Price {current_price:.2f} > MA120 {ma120:.2f} -> Scale 1.0"
